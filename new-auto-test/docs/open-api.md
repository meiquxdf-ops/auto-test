...
- **地址安全限制（SSRF 防护）**：Server 端可配置回调白名单
  `atest.callback.allowed-hosts`（主机名 / IP / CIDR）。
  - 白名单**为空**（默认）：一般公网 http(s) 地址可用，但 loopback、RFC1918 内网、
    link-local（含 `169.254.169.254` 元数据地址）等受保护地址**创建时直接 400**；
  - 白名单**非空**：只有名单内的主机可用（内网回调接收方要显式列进名单）。
  - 校验在创建时和**每次投递前**都会执行；回调**不跟随 3xx 重定向**，重定向响应按
    投递失败处理。接入方给的 callbackUrl 被拒时，找运维把主机加进白名单即可。
- 回调体形如：
